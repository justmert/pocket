...
test("the T10 oracle answers yes on a real blinded balance and no to every perturbation of it", async () => {
  ...
  try {
    ...
    expect(
      stored.spendable.randomness === 0n && stored.receiving.randomness === 0n,
      "a shielded balance must be blinded, or the commitment is a plain function " +
        "of the amount and the oracle is not exercising the blinding term",
    ).toBe(false);
