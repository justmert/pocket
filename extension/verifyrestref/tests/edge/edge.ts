...
export type ReviewOutcome = { stage: "confirm"; text: string } | { stage: "error"; message: string };
...
/**
 * Two clicks on the same button inside ONE task, so the second lands before any
 * re-render can disable or unmount the control.
 *
 ...
 * This one cannot. Both handlers are guaranteed to have run, and it returns how
 * many clicks it dispatched so a test can assert it rather than hope.
 *
 * The button is found by its visible text, which is what a user reads.
 */
export async function clickTwiceInOneTask(page: Page, label: string): Promise<number> {
  ...
  expect(fired, `"${label}" had to be clickable twice for this test to mean anything`).toBe(2);
  return fired;
}
