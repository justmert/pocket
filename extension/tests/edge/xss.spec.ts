...
test("an amount refusal does not read the typed value back onto the screen", async ({ wallet }) => {
  ...
  // refused. `InvalidAmountError` is on `describeError`'s allowlist, which means
  // its message reaches the screen verbatim, and it interpolates the string the
  // user typed. That is the one thing the allowlist exists to prevent, and the
  // fact that the string is the user's own rather than an RPC's bounds the harm
  // without changing the rule.
  for (const value of ["A".repeat(100_000), "<img src=x onerror=alert(1)>", "${alert(1)}"]) {
    await compose(page, { to: valid(), amount: value });
    const out = await review(page);
    expect(out.stage, `${value.slice(0, 20)} must not reach the confirm screen`).toBe("error");
    const said = out.stage === "error" ? out.message : "";
    expect(
      said.length,
      `an amount refusal must stay a sentence, not become the ${value.length} characters that were typed`,
    ).toBeLessThan(1_000);
    expect(said, "an amount refusal must not repeat what was typed").not.toContain(
      value.slice(0, 40),
    );
    await closeSend(page);
  }
