...
The bump is a signed transaction, so a locked wallet cannot make one. A browser that stays closed for a month cannot either.

Pocket states this rather than promising otherwise. Nothing about dormancy loses money, and the recovery is a fee.
...
Nothing about dormancy should ever be calibrated against the testnet figure, because the mainnet one is far more forgiving and a schedule tuned for testnet is merely conservative there.
