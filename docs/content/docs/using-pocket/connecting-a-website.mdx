...
Granting a connection is not wired into the interface in this build, so nothing appears there yet, and the sheet says so rather than describing a flow you cannot reach. Until it is, every method that needs a session answers:

> This site is not connected to Pocket. Open Pocket and connect it first.

The surface fails closed: a site cannot cause a connection prompt to appear, so a page cannot spam you into granting one.
