...
Before the submission outcomes were named on the allowlist, that message was replaced with "Try again, and check your connection."
...
The same discipline covers the diverged screen. Crediting inbound transfers wraps its whole body in a catch that replaces any foreign message, because without it an RPC's own "Request failed with status code 429" appeared on the screen telling somebody their balances did not match the ledger.
...
A catch that collapsed the second into the first removed an entire feature from the screen during an outage, which made a working build look like one that never had it.
