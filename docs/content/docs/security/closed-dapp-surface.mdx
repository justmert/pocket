...
Granting is not wired into the interface in this build, so no origin can obtain a session and every method that needs one answers with the not-connected refusal.

**Settings → Connected sites** lists live grants and removes them, and says plainly that nothing will appear there yet rather than describing a flow you cannot reach.
