...
The exact-length checks matter more than a lower bound. Both writers produce exactly 48 bytes, so any other length is damage rather than a shorter key. A range check let every truncation from 33 to 47 bytes through to the tag test, where it failed and was reported as a wrong password.
