...
[The full leak table, and a real transfer decoded](/docs/overview/privacy-truth).
...
| Assets, public pocket | XLM, USDC, and any classic asset you add a trustline for |
| Assets, private pocket | XLM, USDC |
...
| Swap | XLM and USDC, through Aquarius |
| Yield | deposit and withdraw, through DeFindex |
| Cross-chain USDC | send and claim, through Circle CCTP |
| Website connections | SEP-43 surface implemented. Granting a connection is not wired into the interface in this build |
| Selective disclosure | primitives only. No producer, no verifier, no circuit |
| Multi-account | not built |
...
| Proving time | about 8 seconds for a register operation, measured |
